...
"""
CEFR-targeted sentence simplifier (majority voting) with:
- semantic preservation (SBERT cosine floor),
- reference-aware scoring (SBERT to ref + lightweight SARI),
- reconciliation retries,
- final "nearest-level" fill for any remaining items,
- and sorted output by text_id (01-a1, 01-a2, 01-b1, 01-b2, 01-c1, 01-c2, 02-a1, ...).

OUTPUT FORMAT (JSONL):
  {"text_id": "...", "simplified": "..."}

Run:
  python simplify_and_reconcile.py --input tsar2025_trialdata.jsonl --output Trial_Simplification.jsonl
"""
...
def nearest_level_fill(row: Dict, sim_floor: float, max_steps: int) -> Optional[str]:
    """
...
def write_jsonl_records(path: Path, recs: Iterable[Dict]) -> None:
    with path.open("w", encoding="utf-8") as f:
        for r in recs:
            # r must contain only text_id and simplified
...
def sort_key_text_id(text_id: str) -> Tuple[int, int]:
    """Map '01-a2' -> (1, 1). Unknowns go to the end."""
    m = re.match(r"^(\d+)-([a-c][12])$", text_id, re.IGNORECASE)
    if not m:
        return (10**9, 10**9)
    group = int(m.group(1))
    cefr = (m.group(2)[0] + m.group(2)[1]).upper()  # 'A2'
...
def process_file_with_reconcile(
    ...
    first_out = simplify_batch(data, sim_floor=similarity_floor, max_steps=max_steps, debug=debug)
    for r in first_out:
    ...
    while retry < max_retries and output_ids != input_ids:
        ...
        cur_floor = max(0.75, cur_floor - floor_step)   # lower, but not too low
        cur_steps = min(36, cur_steps + steps_step)     # deeper edits
    ...
    # ---- Final nearest-level fill (no CEFR in output) ----
    if remaining_ids:
        ...
        for miss_id in remaining_ids:
            ...
            if not row:
                continue
            best_text = nearest_level_fill(row, sim_floor=final_floor, max_steps=final_steps)
            if best_text:
                current_records[miss_id] = {"text_id": miss_id, "simplified": best_text}
            else:
                # last-resort conservative fallback: ensure every id is present
...
# ================================= CLI ===================================
def main():
    ap = argparse.ArgumentParser(description="Simplify to target CEFR with reconciliation, nearest-level fill, and sorted output (no CEFR_Level in file).")
